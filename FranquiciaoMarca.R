...
data %<>% tidyr::extract(rotulo, c("extension"), "(S\\.L|S\\.A|\\bSL\\b|\\bSA\\b)", remove = F) %>% mutate(TIPO = extension %in% c("SL", "SA", "S.A", "S.L"))

data$TIPO <- plyr::mapvalues(data$TIPO, from = c("TRUE", "FALSE"), to = c("FRANQUICIA", "MARCA"), warn_missing = F)
data %>% view()
